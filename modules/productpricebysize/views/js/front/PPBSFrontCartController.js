...
PPBSFrontCartController = function() {
	var self = this;
	...
	self.addCustomizations = function() {
		...
		$cart.find('.product-line-grid-body').each(function(i, obj) {
			if ($(obj).find(".customization-modal").length == 0) {
				return;
			}
			...
			$modal.find(".product-customization-line").each(function(index, item) {
				...
				customization_line_html += '<div class="ppbs_customization_line" data-id_product="' + id_product + '" data-id_customization="' + id_customization + '">';
	...
	/**
	 * Remove a cutomization from a product
 	 * @param id_product
	 * @param id_customization
	 */
	/*self.deleteCustomization = function(id_product, id_customization) {
        var url = MPTools.joinUrl(module_ajax_url_ppbs, 'section=front_ajax&route=ppbsfrontcartcontroller&action=deletecustomization');
	...
	/*$('body').on('click', '.ppbs_customization-delete', function () {
		var id_product = $(this).parents(".ppbs_customization_line").attr("data-id_product");
		var id_customization = $(this).parents(".ppbs_customization_line").attr("data-id_customization");
		self.deleteCustomization(id_product, id_customization);
		return false;
	});*/
