...
class PPBSControllerCore extends Module
{
    ...
    public function __construct($sibling, $params = array())
    {
        ...
        $link = new Link();
        $this->module_ajax_url = $link->getModuleLink('productpricebysize', 'ajax', array());
        $this->default_lang = (int)Configuration::get('PS_LANG_DEFAULT');

        if (AdminController::$currentIndex != '') {
            $this->admin_url = $this->context->link->getAdminLink('', false);
        }
    ...
    {
        return Tools::getShopProtocol() . Tools::getShopDomain() . __PS_BASE_URI__ . 'modules/' . $this->sibling->name . '/';
