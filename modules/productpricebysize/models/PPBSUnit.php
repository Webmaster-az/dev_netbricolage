...
class PPBSUnit extends ObjectModel
{
    ...
    /**
     * remove unit and all relationships
     * @param $id
     */
    public static function deleteFull($id_ppbs_unit)
    {
        $sql = '
			...
			WHERE pf.id_ppbs_unit = ' . (int)$id_ppbs_unit;

        $result = Db::getInstance()->execute($sql);
        if ($result) {
            DB::getInstance()->delete('ppbs_unit', 'id_ppbs_unit=' . (int)$id_ppbs_unit);
