...
var PriceTable = function (options)
{
    ...
    this.onLoad = function ()
    {
        ...
        prestashop.on('updateProduct', function (event) {
            ...
            $.post(event.productUrl, {ajax: '1', action: 'refresh'}, null, 'json').then(function (resp) {
                ...
                setTimeout(function(){
                    ...
                    var pc_mainprodprice = $('.pc-pricedisplay .current-price span').attr('content');
                    var pc_mainprodpricecopy = "€ " + pc_mainprodprice.replace(/\B(?=(\d{3})+(?!\d))/g, ",");

                    var pc_elementreplaceprice = $('.accessories_table_price_content tr:last').prev();
                    $(pc_elementreplaceprice).children('.right-column').text(pc_mainprodpricecopy);

                    $('.accessories_table_price_content .right-column').each(function() {
                        pc_pricearr.push(parseFloat($(this).text().replace(/\D*/, "")));
                    });
                    
                    
                
                    var pc_acesstotal = 0;
                    for (var i = 0; i < pc_pricearr.length; i++) {
                        pc_acesstotal += pc_pricearr[i];
                    }

                    $('.accessories_table_price_content .right-column-total').text("€ " + parseFloat(pc_acesstotal).toFixed(2));
...

    {
        ...
        //accessories_price_pc.push({'teste': PriceTable.instance._hsmaFormatCurrency(productPrice)});
...

    {
        ...
        return product.is_available_when_out_of_stock ? '' : '';
