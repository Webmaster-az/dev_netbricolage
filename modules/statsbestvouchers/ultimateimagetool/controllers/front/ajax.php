...
class ultimateimagetoolajaxModuleFrontController extends ModuleFrontController
{
	...
	private function ajax_convert_in_folder()
	{
		...
		if(file_exists($image_path))
		{
			...
			if((int)Configuration::get('uit_use_external_api') == 1 || (substr_count(strtolower($image_path), '.png') > 0 && $is_imagick  == false) ){
				$imagewebp = false;
				$imagick = false;
				$is_imagick = false;
			}
	...
	private function ajax_delete_images()
	{
		...
		$images = UltimateImageTool::search_for_images_in_folder($folder, $size, true);
