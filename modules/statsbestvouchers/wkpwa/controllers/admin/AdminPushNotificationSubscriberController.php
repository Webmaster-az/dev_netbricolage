...
class AdminPushNotificationSubscriberController extends ModuleAdminController
{
    ...
    public function getCustomerInfo($customerName, $params)
    {
        if ($params['id_customer']) {
            if (_PS_VERSION_ >= '1.7.6.0') {
                ...
                $adminLinkCustomer = $this->context->link->getAdminLink(
                    'AdminCustomers',
                    true,
                    array('viewcustomer' => 1, 'id_customer' => $params['id_customer']),
                    array()
                );
            } else {
                $adminLinkCustomer = $this->context->link->getAdminLink('AdminCustomers').'&id_customer='.
                $params['id_customer'].'&viewcustomer';
            }
            $this->context->smarty->assign(
                array(
                    'element_type' => 5,
                    'adminLinkCustomer' => $adminLinkCustomer,
                    'customer_name' => $params['customer_name'],
                    'customer_email' => $params['customer_email']
                )
            );
            $customerLink = $this->context->smarty->fetch(
    ...
    public function getCustomerTotalPaid($idCustomer)
    {
        if ($idCustomer) {
            ...
            $this->context->smarty->assign(array('element_type' => 4, 'customerOrderTotal' => $customerOrderTotal));
