...
class WkPwaPushNotification extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'body' => array('type' => self::TYPE_STRING, 'required' => true, 'lang' => true),
            ...
            'order_status' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
    ...
    public function isOrderNotificationExists($orderStatus)
    {
        $sql = 'SELECT `id` FROM `'._DB_PREFIX_.'wk_pwa_push_notification`
        WHERE `order_status`= '.(int) $orderStatus .'
        AND `id_notification_type` = '.(int) self::ORDER_STATUS_NOTIFICATION;
        return Db::getInstance()->getValue($sql);
    }

    public static function getAssignedOrderStatus()
    {
        $result = Db::getInstance()->executeS('SELECT `order_status` FROM `'._DB_PREFIX_.'wk_pwa_push_notification`');
        if (!empty($result)) {
            return array_column($result, 'order_status');
        }
        return false;
    }
    ...
    {
        $objPushNotification = new self($idPushNotification);
        if (Validate::isLoadedObject($objPushNotification)) {
            return (array) $objPushNotification;
        }
    ...
    public function getPayloadData($idPushNotification, $idElement = 0, $idLang = 0)
    {
        if (!$idLang) {
        ...
        if ($objPushNotification->id_notification_type == self::MANUAL_NOTIFICATION
            ...
            if (!Validate::isLoadedObject($product)) {
                ...
                $this->replaceStringByValue($objPushNotification->title[$idLang], $varValArr);
                $this->replaceStringByValue($objPushNotification->body[$idLang], $varValArr);
            ...
            $orderState = new OrderState($order->current_state, $idLang);
            $varValArr = array(
        ...
        $payload = array(
            'title' => $objPushNotification->title[$idLang],
            'body' => $objPushNotification->body[$idLang],
    ...
    public function pushNotification(
        ...
        foreach ($subscribers as $subscriber) {
            ...
            // get payload data language wise
            if (isset($subscriber['id_lang']) && $idPushNotification) {
                $payload = $this->getPayloadData($idPushNotification, $idElement, $subscriber['id_lang']);
            }
            if (!isset($payload['error'])) {
    ...
    public function getPushNotificationDefaultVariables()
    {
        ...
        $defaultVariables = array(
            ...
            'current_lang' => Language::getLanguage((int)Context::getContext()->language->id),
        );
    ...
    public function procressPushNotificationFields($fields)
    {
        ...
        if (!$fields['title'][$defaultLangId]) {
            $wkError[] = $this->moduleInstance->l('Notification title is required at least in ', 'WkPwaPushNotification').$objDefaultLanguage['name'];
        } else {
            foreach (Language::getLanguages() as $language) {
                if (!Validate::isCleanHtml($fields['title'][$language['id_lang']])) {
                    $lang = Language::getLanguage((int)$language['id_lang']);
                    $wkError[] = $this->moduleInstance->l('Notification title is not valid in ', 'WkPwaPushNotification').$lang['name'];
                }
            }
        }

        if (!$fields['body'][$defaultLangId]) {
            $wkError[] = $this->moduleInstance->l('Notification body is required at least in ', 'WkPwaPushNotification').$objDefaultLanguage['name'];
        } else {
            foreach (Language::getLanguages() as $language) {
                if (!Validate::isCleanHtml($fields['body'][$language['id_lang']])) {
                    $lang = Language::getLanguage((int)$language['id_lang']);
                    $wkError[] = $this->moduleInstance->l('Notification body is not valid in ', 'WkPwaPushNotification').$lang['name'];
                }
            }
        }
        ...
        if (isset($fields['orderStatus'])) {
            if (!$fields['orderStatus'] &&
                ...
                $wkError[] = $this->moduleInstance->l('Please select order status for sending push notification on order status update.', 'WkPwaPushNotification');
            }
        } else {
            $fields['orderStatus'] = array();
        }

        if ($fields['idNotificationType'] == WkPwaPushNotification::ORDER_STATUS_NOTIFICATION) {
            if ($fields['orderStatus'] && !$fields['edit']) {
                if ($this->isOrderNotificationExists($fields['orderStatus'])) {
                    $wkError[] = $this->moduleInstance->l('Notification already exists for this order status.', 'WkPwaPushNotification');
                }
            }
        }
        ...
        if (!count($wkError)) {
            ...
            foreach (Language::getLanguages() as $language) {
                $objPushNotification->title[$language['id_lang']] = $fields['title'][$language['id_lang']];
                $objPushNotification->body[$language['id_lang']] = $fields['body'][$language['id_lang']];
            }
            ...
            $objPushNotification->order_status = $fields['orderStatus'] ? $fields['orderStatus'] : 0;
