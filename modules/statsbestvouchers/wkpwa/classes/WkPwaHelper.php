...
class WkPwaHelper
{
    ...
    public static function generateIosSplashImages($fileSource)
    {
        if (!$fileSource) {
            return false;
        }
        ...
        $imgName = self::_PWA_LOGO_NAME_;

        $imgFolderPath = self::_PWA_MODULE_NAME_.'/views/img/appIcon/';
        foreach ($imgSizes as $size) {
            $imgPath = _PS_MODULE_DIR_.$imgFolderPath.$imgName.'-'.$size[x].'x'.$size[y].'.png';
            ImageManager::resize($fileSource, $imgPath, $size[x], $size[y], 'png');
        }
        $imgPath = _PS_MODULE_DIR_.$imgFolderPath.$imgName.'_'.time().'.png';
        ImageManager::resize($fileSource, $imgPath);
    ...
    public function getWkPwaTables()
    {
        return array(
            ...
            "INSERT INTO `"._DB_PREFIX_."wk_pwa_push_notification_type`
                ...
                (3,	'Order Status Notification', 1,	'CURDATE()',	'CURDATE()'),
    ...
    public function deleteWkPwaTables()
    {
        return Db::getInstance()->execute(
            ...
            `'._DB_PREFIX_.'wk_pwa_push_notification_lang`,
