...
*/
    $(document).ready(function () {
        ...
        $('body').on('click','.editreliquat', function() {
            ...
            $('#editreliquat input.ec_comprimento').val(info_reliquat.comprimento);
            $('#editreliquat input.ec_largura').val(info_reliquat.largura);
            $('#editreliquat input.ec_altura').val(info_reliquat.altura);
            $('#editreliquat input.ec_peso').val(info_reliquat.peso);
        ...
        $('body').on('click','#btneditreliquat', function() {
            ...
            comprimento = $('#editreliquat input.ec_comprimento').val();
            largura = $('#editreliquat input.ec_largura').val();
            altura = $('#editreliquat input.ec_altura').val();
            peso = $('#editreliquat input.ec_peso').val();
        ...
        $('body').on('click','.deletereliquatproduct', function() {
            let confirmAction = confirm("Apagar embalagem?");
            if (confirmAction == true) {
                id_reliquat = $(this).attr('data-reliquat');
                tr = $(this).parent().parent();
                $.ajax({
                    url: ec_base_uri+"/modules/ec_reliquat/ajax.php",
                    type: "POST",
        ...
        $('body').on('click','.deleteproduct', function() {
            let confirmAction = confirm("Apagar producto da embalagem?");
            if (confirmAction == true) {
                id_reliquat_product = $(this).attr('data-reliquat-product');
                $.ajax({
                    url: ec_base_uri+"/modules/ec_reliquat/ajax.php",
                    type: "POST",
                    data: ({
                        majsel: '4',
                        token: ec_token,
                        id_reliquat_product: id_reliquat_product,
                    }),
                    dataType: "json"
                })
                .done(function (data) {
