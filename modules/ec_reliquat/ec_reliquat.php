...
class Ec_reliquat extends Module
{
    ...
    public function hookDisplayAdminOrderMain($params)
    {
     ...
     /*   if (count($prods) == 1) {
        ...
        $reliquats = Db::getInstance()->executeS(
            '
            SELECT id_order, id_reliquat, osl.name as order_state, c.name as carrier, tracking_number, date_add, er.id_carrier, er.id_order_state, comprimento, largura, altura, er.peso, num_albaran
            ...
            LEFT JOIN '._DB_PREFIX_.'gls_envios ge ON (ge.id_envio_order = er.id_order)
            WHERE id_order = '.(int)$id_order.'
            '
        );
    ...
    public function addReliquat($id_order)
    {
        ...
        $id_reliquat = self::insertReliquat($id_order, $id_order_state, $id_carrier, $tracking_number, $comprimento, $largura ,$altura ,$peso);
    ...
    public static function insertReliquat($id_order, $id_order_state, $id_carrier, $tracking_number, $comprimento, $largura ,$altura ,$peso)
    {
        Db::getInstance()->insert(
            ...
            array(
                ...
                'comprimento' => pSQL($comprimento),
                'largura' => pSQL($largura),
                'altura' => pSQL($altura),
                'peso' => pSQL($peso)
            )
        );
    ...
    public function updateReliquat($id_reliquat, $tracking_number, $comprimento, $largura, $altura, $peso, $id_carrier, $id_order_state, $id_order)
    {
    ...
    public function deleteReliquatProducts($id_reliquat)
    {
        return Db::getinstance()->delete('ec_reliquat_product', 'id_reliquat = '.(int)$id_reliquat);
    }
    public function deleteReliquat($id_reliquat)
    {
        return Db::getinstance()->delete('ec_reliquat', 'id_reliquat = '.(int)$id_reliquat);
    }
    public function deleteReliquatProduct($id_reliquat_product)
    {
        return Db::getinstance()->delete('ec_reliquat_product', 'id_reliquat_product = '.(int)$id_reliquat_product);
    }  
    ...
    public function hookDisplayOrderDetail($params)
    {
        ...
        $reliquats = Db::getInstance()->executeS(
            '
            SELECT id_order, id_reliquat, osl.name as order_state, c.name as carrier, tracking_number, comprimento, largura, altura, er.peso, date_add, er.id_carrier, num_albaran
            FROM '._DB_PREFIX_.'ec_reliquat er
            LEFT JOIN '._DB_PREFIX_.'carrier c ON (er.id_carrier = c.id_carrier)
