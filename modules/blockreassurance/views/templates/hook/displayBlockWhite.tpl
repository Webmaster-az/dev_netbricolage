...
<div class="blockreassurance col-sm-12">
    ...
    {foreach from=$blocks item=$block key=$key name=blocks}
        ...
        {if $idxCol > ($numCols - $numColsRemaining_sm)}
            ...
            {/if}
        {/if}
        <a class="col-md-{$sizeCol_md} {$offsetCol_md} col-sm-{$sizeCol_sm} {$offsetCol_sm} col-xs-12"
            {if $block['type_link'] !== $LINK_TYPE_NONE && !empty($block['link'])} style="cursor:pointer;" href="{$block['link']}"{/if}>
