...
{
	jQuery('.carrier-extra-content').toggleClass('carrier-extra-content carrier-extra-content-asm');
	...
	$('button[name=processCarrier]').live('click',function(event) 
	{
		if(jQuery(".delivery_option_radio").length)
			selectedCarrier = parseInt($('input[type=radio].delivery_option_radio:checked').val().replace(',', ''));
		else
			selectedCarrier = parseInt($('input[type=radio][name=id_carrier]:checked').val().replace(',', ''));
      	
		if (selectedCarrier == parseInt(jQuery('#parcelshopid').val()) && jQuery('#parcel_codigo').val() == ''){
			event.preventDefault();
			var msg = '<div class="panel panel-info"><div class="panel-heading"><h3 class="panel-title">Debe seleccionar un punto de recogida</h3></div>'
			document.getElementById("parcelinfo").innerHTML  = msg;
			event.preventDefault();
