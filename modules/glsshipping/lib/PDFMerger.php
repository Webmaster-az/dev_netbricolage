...
class PDFMerger
{
    ...
    $fpdi->SetPrintFooter(false);
		...
		if($mode == 'S')
		{
			return $fpdi->Output(basename($outputpath), 'S');
		}
		else if($mode == 'F')
		{
		...
		{
			if($fpdi->Output(basename($outputpath), $mode) == '')
			{
				return true;
			}
			else
...
public function mergeApli4($outputmode = 'browser', $outputpath = 'newfile.pdf')
	{
		if(!isset($this->_files) || !is_array($this->_files)): throw new exception("No PDFs to merge."); endif;
				...
				for($i=1; $i<=$count; $i++)
				{
					...
					switch ($pdfcount){
						case 0:
							$posx=0;
							$posy=0;
							$fpdi->AddPage($orientation, array($size['w']*2, $size['h']*2));
							$pdfcount++;
							break;
						case 1:
							$posx=$size['w'];
					...
					$fpdi->useTemplate($template, $posx, $posy, $size['w'], $size['h']);
				}
			}
			else
			{
