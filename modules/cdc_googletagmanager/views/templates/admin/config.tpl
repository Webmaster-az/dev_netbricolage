				...
				<div role="tabpanel" class="tab-pane active panel" id="tagmanager">
					<div class="panel-body">
							...
							<div class="margin-form col-lg-3 form-inline">
								<select class="form-control" id="CDC_GTM_DATALAYER_FORMAT" name="CDC_GTM_DATALAYER_FORMAT">
									{foreach from=$CDC_GTM_DATALAYER_FORMAT_LIST key=k item=v}
										<option value="{$k|escape:'htmlall':'UTF-8'}" {if $k == $CDC_GTM_DATALAYER_FORMAT}selected="selected"{/if}>{$v|escape:'htmlall':'UTF-8'}</option>
									{/foreach}
								</select>
							</div>
							<div class="col-lg-6">
						...
						<div class="form-group">
							...
							<div class="col-lg-6">
								<p class="cdc-info">{l s='If the datalayer was not created during the order (no redirection to the confirmation page for example), the datalayer can be automatically created. Be careful if you enable this option, orders created from your backoffice and imported from external sources (marketplaces) will be tracked in your analytics.' mod='cdc_googletagmanager'}</p>
							</div>
						</div>
