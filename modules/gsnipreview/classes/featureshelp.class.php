...
class featureshelp {
    ...
    public function __construct(){
        ...
        $this->_table_data_order = 'gsnipreview_data_order';
        $this->_table_customer = 'gsnipreview_customer';
    ...
    public function getOrderInfo($data){
        ...
        foreach ($data_product_in_order as $product) {
            ...
            $id_pr = isset($attributes['id_product'])?$attributes['id_product']:0;
            if(!$id_pr) continue;


            if(version_compare(_PS_VERSION_, '1.6', '>')) {
                $link = Context::getContext()->link;
                $product_obj = new Product($id_pr);
    ...
    public function getCronTaskDelayForReminder($data){
        ...
        ### get products ids in order ###
        $data_info_about_order = $this->getOrderInfo(
            array('order_id'=>$id_order,

            )
        );
        $data_products = $data_info_about_order[0]['products'];
        $data_tmp_ids = array();
        foreach($data_products as $_product) {
            $id_product = $_product['id_product'];
            $data_tmp_ids[] = $id_product;
        }
        ### get products ids in order ###
        ...
        if(Configuration::get($this->_name.'reminder')==0){
            ...
            if(!$remrevsec) {
                ...
                $is_add_review = $obj_shopreviews->isExistsReviewByCustomer(array('id_customer' => $customer_id,'id_product'=>$data_tmp_ids));
    ...
    public function sendCronTab($data_in = null){
        ...
        $tasks_send = 0;
        $orderstatuses = Configuration::get($this->_name.'orderstatuses');
            ...
            foreach ($data_tasks as $task) {
                ...
                if (!empty($data_task) && is_array($data_task)) {
                    ...
                    foreach ($data_task as $k1=> $product ) {
                        ...
                        #### addd image ###
                        $_obj_product = new Product($id_product);
                        $data_product = $obj->_productData(array('product' => $_obj_product, 'id_lang' => $id_lang, 'block'=>'reminder'));
                    ...
                    if(!$remrevsec) {
                        $is_add_review = $obj_shopreviews->isExistsReviewByCustomer(array('id_customer' => $task['customer_id'],'id_product'=>$tmp_arrray));
                        if ($is_add_review)
                            continue;
                    }
                    // only for customers who not add review //
    ...
    public function sendNotificationConfirmationByAdmin($data){
        ...
        if(version_compare(_PS_VERSION_, '1.6', '>')){
            ...
            Mail::Send($id_lang_current, 'customer-reminder-admin-'.$this->_prefix, $data['subject'], $data['vars'],
                Configuration::get($this->_name.'mail'), 'Admin confirmation form', NULL, NULL, NULL, NULL, dirname(__FILE__).'/../mails/', NULL, $id_shop);
        } else {
            /* Email sending */
            Mail::Send($id_lang_current, 'customer-reminder-admin-'.$this->_prefix, $data['subject'], $data['vars'],
                Configuration::get($this->_name.'mail'), 'Admin confirmation Form', NULL, NULL, NULL, NULL, dirname(__FILE__).'/../mails/');
        }
    ...
    public function sendNotification($data){
        ...
        if(!empty($data['email'])) {
            if (version_compare(_PS_VERSION_, '1.6', '>')) {
                /* Email sending */
                Mail::Send($id_lang_current, 'customer-reminderserg', $data['subject'], $data['vars'],
        ...
        if (!empty($orders)) {
            ...
            foreach ($orders as $order) {
                ...
                if (Validate::isLoadedObject($obj_order)) {
                    ...
                    //echo "<pre>"; var_dump($obj_order);exit;
