...
class ProductCore extends ObjectModel
{
    ...
    public $pc_freeshipping = false;

    /** @var string multiplication */
    ...
    public static $definition = [
        ...
        'fields' => [
            /* Custom fields */
            'multiplication' => array('type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isString'),
            'warranty' => array('type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isString'),
            ...
            //'deliverytime' => ['type' => self::TYPE_STRING, 'validate' => 'isReference', 'size' => 64],
            ...
            'pc_freeshipping' => ['type' => self::TYPE_BOOL, 'shop' => true, 'validate' => 'isBool'],
    ...
    public function update($null_values = false)
    {
        ...
        if (Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT') && StockAvailable::dependsOnStock($this->id, Context::getContext()->shop->id)) {
            Db::getInstance()->update('stock', [
                ...
                'deliverytime' => pSQL($this->deliverytime),
    ...
    public function updateAttribute(
        ...
        $combination->deliverytime = pSQL($deliverytime);
        ...
        if (Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT') && StockAvailable::dependsOnStock($this->id, Context::getContext()->shop->id)) {
            Db::getInstance()->update('stock', [
                ...
                'deliverytime' => pSQL($deliverytime),
                'ean13' => pSQL($ean13),
                'isbn' => pSQL($isbn),
                'upc' => pSQL($upc),
                'mpn' => pSQL($mpn),
    ...
    public static function getNewProducts($id_lang, $page_number = 0, $nb_products = 10, $count = false, $order_by = null, $order_way = null, Context $context = null)
    {
        ...
        $sql = new DbQuery();
        $sql->select(
            'p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, pl.`multiplication`, pl.`warranty`, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`,
    ...
    */
    ...
    public static function getAttributesParams($id_product, $id_product_attribute)
    {
        ...
        if (!Cache::isStored($cache_id)) {
            $result = Db::getInstance()->executeS('
            SELECT a.`id_attribute`, a.`id_attribute_group`, al.`name`, agl.`name` as `group`, pa.`reference`,pa.`deliverytime`, pa.`ean13`, pa.`isbn`, pa.`upc`, pa.`mpn`
    ...
    {
        $result = Db::getInstance()->executeS('
        SELECT DISTINCT a.`id_attribute`, a.`id_attribute_group`, al.`name` as `attribute`, agl.`name` as `group`,pa.`reference`,pa.`deliverytime`, pa.`ean13`, pa.`isbn`, pa.`upc`, pa.`mpn`
        FROM `' . _DB_PREFIX_ . 'attribute` a
        LEFT JOIN `' . _DB_PREFIX_ . 'attribute_lang` al
