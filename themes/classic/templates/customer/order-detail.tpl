...
{block name='page_content'}
  {block name='order_infos'}
    <div id="order-infos" style="background-color: #f9f9f9;">
      <div class="box">
          <div>
            <div class="col-12 order-pad">
                Commande nº : <span style="font-weight: bold;">{$order.details.reference}</span>
            </div>
            
            <div class="col-12 order-pad">
                Date d'achat : <span style="font-weight: bold;">{$order.details.order_date}</span>
            </div>
            
            <!--
            <div class="col-12 order-pad">
              {l s='Carrier' d='Shop.Theme.Checkout'} : <span style="font-weight: bold;">{$order.carrier.name}</span>
            </div>
            -->
            <div class="col-12 order-pad">
              {l s='Payment method' d='Shop.Theme.Checkout'} : <span style="font-weight: bold;">{$order.details.payment}</span>
            </div>

            <div class="col-12 order-pad">
              {$order.totals.total.label} : <span style="font-weight: bold;">{$order.totals.total.value}</span>
            </div>
            ...
            {if $order.details.reorder_url}
              <div class="col-12 order-pad">
                <a href="{$order.details.reorder_url}" class="button-primary">              
  ...
  {block name='order_history'}
    <section id="order-history" class="box">
      ...
      -->
      {foreach from=$order.history item=state name=historyLoop}
        
        <div class="order-track">
          <div class="order-track-step">
            <div class="order-track-status">
              <span class="order-track-status-dot{if $smarty.foreach.historyLoop.first}-last{/if}"></span>
              <span class="order-track-status-line{if $smarty.foreach.historyLoop.last}-last{/if}"></span>
            </div>
            <div class="order-track-text">
              <p style="font-weight: bold;">{$state.ostate_name}</p>
              <span>{dateFormat date=$state.date_add full=true}</span>
            </div>
          </div>
        </div>
      {/foreach}
