...
final class GetOrderForViewingHandler extends AbstractOrderHandler implements GetOrderForViewingHandlerInterface
{
    ...
    private function getOrderMessages(Order $order): OrderMessagesForViewing
    {
        ...
        $customerThreadId = $this->customerDataProvider->getCustomerThreadId(
            (int) $order->id_customer,
            (int) $order->id
        );
        ...
        foreach ($customerThreadId as $idcustomerThread){
            $id_customerThread = (int) $idcustomerThread['id_customer_thread'];
        }

        $token = '0';

        $token = Tools::getAdminTokenLite('AdminCustomerThreads');
        ...
        return new OrderMessagesForViewing($messages, $orderMessagesForOrderPage['total'], $id_customerThread, $token);
    }
