...
class CustomerDataProvider
{
    ...
    /**
     * Provides customer Thread Id
     *
     * @param int $customerId
     */
    public function getCustomerThreadId(int $customerId, int $orderId)
    {
        $customerThreadId = 0;
        $Sql = 'SELECT ct.`id_customer_thread`
            ...
            LEFT JOIN ' . _DB_PREFIX_ . 'customer c
                ON ct.`id_customer` = c.`id_customer`
            LEFT OUTER JOIN ' . _DB_PREFIX_ . 'orders o
                ON o.`id_order` = ct.`id_order`
            WHERE ct.`id_customer` = ' . $customerId . ' AND ct.`id_order` = ' . $orderId;
        
        $customerThreadId = Db::getInstance()->executeS($Sql);
